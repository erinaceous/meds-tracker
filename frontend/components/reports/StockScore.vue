...
export default defineComponent({
  ...
  computed: {
    ...
    scoresInterpolated() {
      ...
      const lastScore = this.scoresDated[this.scoresDated.length - 1];
      const newScores = [];
      let currentScore = this.scoresDated[0];
      const date = new Date(currentScore.reported_for_date);
      while (true) {
        const nextScore = this.scoresDated.find(
          (score) =>
            score.reported_for_date.toLocaleDateString() ===
    ...
    <div class="d-flex justify-center align-center">
      <div class="d-block">
        <v-card-title class="text-wrap">
          <div v-if="currentScore === undefined">No reports yet</div>
          <div v-else-if="currentScore <= 0.33">Low stock reported</div>
          <div v-else-if="currentScore > 0.66">Reported in stock</div>
          <div v-else>Might be in stock</div>
          <div v-if="currentScore !== undefined">
            Score: <strong>{{ currentScore.toFixed(2) }}</strong>
          </div>
        </v-card-title>
        <v-card-subtitle
          v-if="report?.scores?.length === 1"
          class="text-wrap mb-2"
        >
          Based on <strong>1</strong> report on
          <strong>{{ report?.scores[0].reported_for_date }}</strong>
        </v-card-subtitle>
        <v-card-subtitle
          v-else-if="report?.scores?.length > 1"
          class="text-wrap mb-2"
        >
          Based on <strong>{{ report?.scores.length }}</strong> reports from
          <strong>{{ report?.scores[0].reported_for_date }}</strong> until
          <strong>{{
            report?.scores[report?.scores.length - 1].reported_for_date
          }}</strong>
          <ul class="ml-4">
            <li />
          </ul>
      ...
      <v-spacer />
      <v-icon class="ma-4" size="48" :color="scoreColour">
        {{ scoreIcon }}
      </v-icon>
    </div>
    <v-sparkline
      ...
      type="trend"
      :gradient="[
        $vuetify.theme.current.colors.success,
        $vuetify.theme.current.colors.warning,
        $vuetify.theme.current.colors.error,
      ]"
    />
  </v-sheet>
</template>
