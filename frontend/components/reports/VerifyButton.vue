...
export default defineComponent({
  ...
  computed: {
    finalColor() {
      if (this.retryAuth && this.retryColor && this.authenticating === 'error') {
    ...
    finalAppendIcon() {
      if (this.retryAuth && this.appendRetryIcon && this.authenticating === 'error') {
        return this.appendRetryIcon
      }
      return this.appendIcon
    },
    finalLoading() {
      return this.loading || (this.authenticating && this.authenticating !== 'error')
    }
  },
  mounted() {
  ...
  methods: {
    ...
    async checkSignature(signature) {
      let result = await fetch(
          `${this.$config.public.api.root}/altcha`,
          {
            headers: {
    ...
    async beginSubmission() {
      let signature = this.$localStorage.getItem('signature');
      if (signature) {
        this.authenticating = 'checking';
        let valid = await this.checkSignature(signature);
        if (valid) {
    ...
    altchaVerification(event) {
      this.$localStorage.setItem('signature', event.detail.signature, event.detail.expires);
