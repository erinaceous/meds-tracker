...
export default defineComponent({
  ...
  beforeMount() {
    this.fetchItemsDebounced = useDebounce(
        this.fetchItems, 500
    );
  },
  mounted() {
  ...
  methods: {
    async getItems(search) {
      ...
      return $fetch(
          this.autocompleteUrl.replace('${search}', search)
      )
    },
    ...
    async searchItems(search) {
      if (!search || search === '' || search === null || search.length < this.minCharacters) {
...
<template>
  ...
  <component :is="component"
      ...
      return-object
      no-filter
      :hide-no-data="(items === undefined) || (!search || search?.length < minCharacters)"
  >
    <template #append-item v-if="items !== undefined && items.length === 0">
    ...
    <template #selection="{ item }">
      <slot name="selection" v-bind="{ item }">
        <v-list-item class="d-flex">
          ...
          <template #append>
            <div v-if="model instanceof Array && model?.length > 1" class="text-grey ml-8">
              <h4>&plus;</h4>
            </div>
          </template>
        </v-list-item>
    ...
    <template #item="{ props, item }">
      <slot name="item" v-bind="{ props, item }">
        ...
        >
          ...
          <slot name="item-text" v-bind="{ item }"></slot>
        </v-list-item>
      </slot>
    </template>
  </component>
