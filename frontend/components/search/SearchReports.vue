...
export default defineComponent({
  ...
  beforeMount() {
    this.searchReportsDebounced = useDebounce(this.searchReports, 250);
  },
  mounted() {
    this.loading = false;
  },
  methods: {
    ...
    async getReports(
      ...
      return $fetch(`${this.$config.public.api.root}/reports/scores`, {
        params: {
...
<template>
  <v-card rounded="lg" elevation="24">
    <v-card-title> Search for stock reports </v-card-title>
    <v-list density="compact">
      ...
      <v-list-subheader
        >At these dosages
        <span class="text-grey">(optional)</span>:</v-list-subheader
      >
      <v-list-item>
        <v-autocomplete
      ...
      <v-list-subheader> For the past: </v-list-subheader>
      <v-list-item>
