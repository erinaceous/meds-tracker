...
START_PAGE = (
    "/api/3/action/datastore_search?resource_id=CONSOL_PHARMACY_LIST_202324Q4&limit=100"
)
CACHE_DIR = os.path.join(os.getcwd(), "data", "download", "pharmacies")
POSTCODES_FILE = os.path.join(os.getcwd(), "data", "download", "postcodes.json")
    ...
    if offset is None:
        for o in range(100, result.get("total"), 100):
            yield from get_pharmacies(
                page=page, cache_dir=cache_dir, root=root, offset=o
            )
...
def get_postcodes():
    ...
    return {
        ...
        .replace(" ", "")
        .strip()
        .upper(): {"latitude": postcode[1], "longitude": postcode[2]}
...
def main():
    ...
    for pharmacy in get_pharmacies(cache_dir=args.cache_dir):
        ...
        if postcode in pharmacy_postcodes:
            continue
        pharmacy_postcodes.add(postcode)
        location = postcodes.get(postcode, {})
        pharmacies.append(
            {
                "name": pharmacy.get("PHARMACY_TRADING_NAME"),
                "address": "\n".join(
                    [pharmacy.get(f"ADDRESS_FIELD{x}", "") or "" for x in range(1, 5)]
                ).strip(),
