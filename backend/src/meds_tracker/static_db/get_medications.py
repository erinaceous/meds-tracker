...
CACHE_DIR = os.path.join(os.getcwd(), "data", "download", "nice_bnf_drugs")


def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("-c", "--cache-dir", default=CACHE_DIR)
    parser.add_argument("-r", "--api-root", default=ROOT)
...
def request(page=None, root=ROOT, cache_dir=CACHE_DIR):
    ...
    if os.path.isfile(cached_response):
        ...
        os.makedirs(os.path.dirname(cached_response), exist_ok=True)
...
def extract_dosages(product):
    ...
    for regex in DOSAGE_REGEXES.values():
        equivalences.update(regex.findall(doseEquivalence))
    return equivalences
...
def get_medication_names():
    ...
    for slug in index:
        ...
        medications[title] = [
            product.get("contentFor").strip() for product in get_products(meds_group)
        ]
    return medications
